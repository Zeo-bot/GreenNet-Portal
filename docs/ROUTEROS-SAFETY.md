...
The live audit database was separately remediated: 11 audit rows and 28 raw-row structures were sanitized without row deletion or schema change, and the post-remediation detector reports zero findings. Historical backups and old Git objects remain untrusted. `AdminUserManagerPasswordController` has completed isolated migration coverage; its separately authorized live password lifecycle remains pending and must prove redaction across session state, audits, DTOs, temporary SQLite bytes, and the exact live test lifecycle.
