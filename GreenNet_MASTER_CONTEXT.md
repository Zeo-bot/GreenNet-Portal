...
- Phase 1D-E migrates `AdminUserManagerUserCreateController` to the shared lazy read/guarded-write gateways. Preview accepts no password and stores safe user/profile projections only. Execute verifies that the user is absent and profile exists, performs `/user-manager/user/add` then `/user-manager/user-profile/add`, and verifies the exact created user and relation. The isolated suite passes with 181 tests and 594 assertions. Authorized Lab Router validation on identity `hAP` passed create, profile assignment, one real audit, secret-persistence checks, and exact-ID cleanup of both relation and user.
- Phase 1D-F migrates `AdminUserManagerUserDeleteController` to the shared lazy read/guarded-write gateways. It revalidates the exact user and child identifiers, removes verified sessions, then verified user-profile relations, then the verified user inside one guarded callback, and verifies their absence. The isolated suite passes with 188 tests and 629 assertions. Authorized Lab Router validation on identity `hAP` passed creation through the migrated create flow, exact-ID deletion through the migrated delete flow, one delete real audit, absence verification, and cleanup without changing unrelated users or profiles.
- Phase 1E completes the operational Controller boundary migration for User Manager package assignment/replacement, package push, User Manager/Hotspot/PPP disconnect, and the User Manager control-center reads. No production Controller in this operational scope constructs `RouterOSApiClient`, receives `RouterOSClientInterface`, calls `comm()`, asserts real-write safety directly, or owns real-attempt audits. The isolated suite passes with 197 tests and 1281 assertions.
- The authorized Phase 1E Lab Router run on identity `hAP` passed user creation, profile A assignment, exact replacement with profile B, unique package limitation/profile/mapping push, migrated user deletion, exact cleanup, one real audit for each executed logical operation, and zero generated-secret occurrences. No natural test session existed, so live disconnect returned the safe no-session result; all three exact remove commands remain covered in isolation.
...
1. Migrate or retire the intentionally excluded legacy RouterOS service paths through separately scoped read boundaries without broadening access to network or system configuration.
